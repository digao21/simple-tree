...
## 5. Adding Colors and Highlighting to the Filesystem Tree

### User Question
> I want to add some nice colors to the filesystem I'm presenting. How do you suggest me to do that?

### Response

In modern Neovim plugins, the most robust and accurate approach is **Extmarks and Buffer Highlighting** (`vim.api.nvim_buf_set_extmark` / `vim.api.nvim_buf_add_highlight`) in a dedicated namespace, rather than legacy regex-based syntax matching.

#### Key Advantages:
1. **Dynamic Icon Colors:** Both `mini.icons` and `nvim-web-devicons` return an associated highlight group alongside the icon glyph (e.g., `DevIconLua` for Lua, `DevIconMd` for Markdown). Extmarks allow coloring each icon individually.
2. **Deterministic & Pure Domain Output:** The domain renderer computes exact byte offsets without touching `vim.*` APIs, keeping unit tests completely mock-free.
3. **Seamless Theme Integration:** Custom `SimpleTree*` highlight groups link by default (`default = true`) to standard Neovim highlight groups (such as `Directory`, `Title`, `Normal`), ensuring compatibility with any user colorscheme.

---

#### Highlight Group Scheme

Standard highlight groups that users and colorschemes can easily override:

| Highlight Group | Default Link | Description |
| :--- | :--- | :--- |
| `SimpleTreeRoot` | `Title` or `Directory` | The root workspace directory line / header |
| `SimpleTreeDirectory` | `Directory` | Directory names |
| `SimpleTreeFile` | `Normal` or `Identifier` | Regular file names |
| `SimpleTreeFolderIcon` | `Directory` | Default folder icons (``, ``) |
| `SimpleTreeIcon` | Dynamic (e.g. `DevIcon*` / `MiniIcons*`) | Third-party file/folder icons with their resolved color group |

---

#### Architectural Boundary & Data Flow

To maintain strict boundary isolation per Hexagonal Architecture:

```
┌─────────────────────────────────────────────────────────────┐
│                       Domain Layer                          │
│                                                             │
│  lua/simple-tree/ui.lua                                     │
│  - Traverses Node tree                                      │
│  - Extracts icon + hl_group from icon providers             │
│  - Computes byte spans:                                     │
│    { line = 0, col_start = 2, col_end = 6, hl_group = "..." }│
│  - Returns: lines, line_to_id, highlights                   │
└──────────────────────────────┬──────────────────────────────┘
...
│                                                             │
│  lua/simple-tree/infrastructure/window.lua                  │
│  - Initializes SimpleTree default highlight groups          │
│  - Creates namespace: vim.api.nvim_create_namespace(...)    │
│  - Applies extmarks: vim.api.nvim_buf_set_extmark(...)      │
└─────────────────────────────────────────────────────────────┘
```

---

## 6. How Extmarks and Buffer Highlighting Work Together

### User Question
> How extmarks and buffer highlighting works together? I know nothing about it. Give me an explanation with code examples.

### Response

In Neovim, **extmarks (Extended Marks)** and **highlight groups** are the modern, standard way plugins color and decorate buffers.

---

### Core Concepts

1. **Highlight Group (`hl_group`)**: 
   A named style definition in Neovim that controls foreground color, background color, boldness, etc. (e.g., standard groups like `Directory`, `Comment`, `String`, or custom groups like `SimpleTreeDirectory`).
2. **Namespace (`ns_id`)**: 
   An isolated bucket created with `vim.api.nvim_create_namespace("my_plugin")`. All your plugin's highlights and marks live in this namespace, allowing you to clear or update them without touching highlights from other plugins or LSP.
3. **Extmark (`nvim_buf_set_extmark`)**: 
   A tracked position or range in a buffer. When passed an `hl_group` and an `end_col`, Neovim applies that highlight color directly across that byte range.

---

### Key Rules of Neovim Buffer Coordinates

* **0-indexed lines:** Line `1` in Vim is index `0` in the API.
* **0-indexed byte columns:** Column positions are measured in **bytes**, not character counts. 
  *(This is critical for Unicode icons like ``, which take 3 bytes in UTF-8: `string.len("") == 3`).*
...
```lua
-- 1. Create a dedicated namespace for your plugin
local ns_id = vim.api.nvim_create_namespace("simple_tree_highlights")

-- 2. Define custom highlight groups (or link to standard Neovim groups)
-- `default = true` allows colorschemes or users to override your colors
vim.api.nvim_set_hl(0, "SimpleTreeDirectory", { default = true, link = "Directory" })
vim.api.nvim_set_hl(0, "SimpleTreeFolderIcon", { default = true, fg = "#61afef", bold = true })
vim.api.nvim_set_hl(0, "SimpleTreeFile", { default = true, fg = "#abb2bf" })

-- 3. Create a scratch buffer and populate it with lines
local buf = vim.api.nvim_create_buf(false, true)
local lines = {
    " my_project",     -- Line 0 (UTF-8 icon '' is 3 bytes)
    "   src",          -- Line 1 (2 spaces = 2 bytes, '' = 3 bytes)
    "    main.lua",      -- Line 2 (4 spaces = 4 bytes)
}
vim.api.nvim_buf_set_lines(buf, 0, -1, false, lines)

-- 4. Apply highlights using extmarks

-- Line 0: Root directory
-- Highlight folder icon '' (bytes 0 to 3)
vim.api.nvim_buf_set_extmark(buf, ns_id, 0, 0, {
    end_col = 3,
    hl_group = "SimpleTreeFolderIcon",
})
-- Highlight directory name 'my_project' (bytes 4 to 14)
vim.api.nvim_buf_set_extmark(buf, ns_id, 0, 4, {
    end_col = 14,
    hl_group = "SimpleTreeDirectory",
})

-- Line 1: Subdirectory 'src'
-- '   src' -> 2 spaces (0..2), icon (2..5), space (5..6), name (6..9)
vim.api.nvim_buf_set_extmark(buf, ns_id, 1, 2, {
    end_col = 5,
    hl_group = "SimpleTreeFolderIcon",
})
vim.api.nvim_buf_set_extmark(buf, ns_id, 1, 6, {
    end_col = 9,
    hl_group = "SimpleTreeDirectory",
})

-- Line 2: File 'main.lua'
-- Highlight file name (bytes 4 to 12)
vim.api.nvim_buf_set_extmark(buf, ns_id, 2, 4, {
    end_col = 12,
    hl_group = "SimpleTreeFile",
})
```

---

### How to Refresh or Clear Highlights

When a folder is expanded/collapsed and the tree re-renders, you simply clear the namespace and re-apply:

```lua
-- Clears all extmarks in this namespace from line 0 to the end of the buffer (-1)
vim.api.nvim_buf_clear_namespace(buf, ns_id, 0, -1)

-- Set new lines and apply new extmarks
vim.api.nvim_buf_set_lines(buf, 0, -1, false, new_lines)
for _, mark in ipairs(new_highlights) do
    vim.api.nvim_buf_set_extmark(buf, ns_id, mark.line, mark.col_start, {
...
### How this Fits into SimpleTree's Architecture

Because `simple-tree` strictly isolates Neovim APIs from domain logic:

1. **Domain Layer (`lua/simple-tree/ui.lua`)**:
   Does pure string calculations. It calculates the byte offsets for each icon and label and returns plain Lua tables:
   ```lua
   {
       line = 0,
       col_start = 0,
   ...
   ```
2. **Infrastructure Layer (`lua/simple-tree/infrastructure/window.lua`)**:
   Receives this list and calls `vim.api.nvim_buf_set_extmark` on the actual buffer.
