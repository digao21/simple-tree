...
local function getIcon(node)
  if node.type == "directory" then
    if node.expanded then return FOLDER_ICON_OPEN end
  ...
  if devicons_loaded and devicons then return devicons.get_icon(node.name) or "" end
...
local function renderNode(node, prefix, lines, line_to_id)
  ...
  if icon and icon ~= "" then name = icon .. " " .. name end

  table.insert(lines, prefix .. name)
  if node.id then line_to_id[#lines] = node.id end
